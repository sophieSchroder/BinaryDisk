...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  dfloor=pin->GetOrAddReal("hydro","dfloor",1.0e-5);
		  ...
		  const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc, AthenaArray<Real> &cons)
{
  ...
  if(is_restart>0){
    ...
    for(int i=0; i<3; i++){
      xi[i]    = 0.0;//pmb->pmy_mesh->ruser_mesh_data[0](i);
      vi[i]    = 0.0;//pmb->pmy_mesh->ruser_mesh_data[1](i);
      Omega[i] = 0.0;//pmb->pmy_mesh->ruser_mesh_data[2](i);
    }
    // print some info
...
void Mesh::UserWorkInLoop(){
  ...
  */
  ...
  MPI_Bcast(Omega,3,MPI_ATHENA_REAL,0,MPI_COMM_WORLD);
#endif
  */
  /*
