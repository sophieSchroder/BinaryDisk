...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  //try to do integration
  ruser_mesh_data[7].NewAthenaArray(blocksizex2);
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
  ...
  AllocateUserOutputVariables(35); //store two point mass function
  return;
}
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin){
  ...
  for(int k=ks; k<=ke; k++){
	...
	//test integration
       user_out_var(34,k,j,i) = pmy_mesh->ruser_mesh_data[7](i);
...
void Mesh::UserWorkInLoop(){
  ...
  for(int k=ks; k<=ke; k++){
	...
	ruser_mesh_data[6](9,k,j,i) += dt*(vtot/sqrt(cs2_c))*pcoord->dx2f(j);
      ...
      Real rcrossfext = 0.0; //integrate R*F_ext_phi over each ring	
      for(int j=js; j<=je; j++){
        rcrossfext += ruser_mesh_data[3](1,k,j,i)*rad_c*pcoord->dx2f(j); //phi integration of r cross f_ext
      }
      ruser_mesh_data[7](i) = rcrossfext;
    }
  }
