...
//XS: The same as ViscousFluxIso, only for binarydisk_vis.cpp problem, only x1 direction
void HydroDiffusion::ViscousFluxAniso(const AthenaArray<Real> &prim,
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      ...
      FaceXdz(k, j, is, ie+1, prim, fz_);
      for (int i=is; i<=ie+1; ++i) {
        nu1  = 0.5*(nu(DiffProcess::aniso,k,j,i)   + nu(DiffProcess::aniso,k,j,i-1));
        denf = 0.5*(prim(IDN,k,j,i) + prim(IDN,k,j,i-1));
	//XS: only include phi-direction momentum accroos R-direction face
        flx1 = 0.0;//-denf*nu1*(fx_(i) + nuiso2*0.5*(div_vel_(k,j,i) + div_vel_(k,j,i-1)));
        flx2 = -denf*nu1*fy_(i);
