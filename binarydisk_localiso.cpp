...
void KinematicViscosity(HydroDiffusion *phdif, MeshBlock *pmb, const AthenaArray<Real> &prim,const AthenaArray<Real> &bcc, int is, int ie, int js, int je,int ks, int ke);
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  diskscale = 0.0;
  diskalpha = 0.0;
  diskscale = pin->GetReal("problem", "diskscale");
  // Enroll viscosity if specified
  if (pin->GetReal("problem","nu_aniso")>0){
    EnrollViscosityCoefficient(KinematicViscosity);
    diskalpha = pin->GetReal("problem", "diskalpha");
  }
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
  ...
  AllocateUserOutputVariables(39); //store two point mass function
  ...
  ruser_meshblock_data[0].NewAthenaArray(7,blocksizex3, blocksizex2, blocksizex1);
  //store the time averaged quantities
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin){
  ...
  for(int k=ks; k<=ke; k++){
	...
	user_out_var(38,k,j,i) = ruser_meshblock_data[0](6,k,j,i);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k=ks; k<=ke; ++k) {
	...
	Real Temp_iso = (GM1/r_local) * pow(diskscale, 2);
	//apply isothermal temp
        press_init = rho_floor*Temp_iso;
...
void Mesh::UserWorkInLoop(){
  ...
  */
  ...
  for(int k=ks; k<=ke; k++){
	...
	//Doing locally isothermal cooling
	Real IE_now = phydro->w(IPR,k,j,i)/(gamma_gas - 1.0);
	Real Temp_iso = (GM1/rad_c) * pow(diskscale, 2);
	//printf("temp_iso: %g, press: %g \n", Temp_iso, rho_c*Temp_iso);
...
#pragma omp simd
	  ...
	  Real nu_R = diskalpha*diskscale*diskscale*sqrt(GM1*rad);
	  phdif->nu(HydroDiffusion::DiffProcess::aniso,k,j,i) = nu_R;
	}//end i
      }//end j
    }//end k
  }//end if
