		  ...
		  const AthenaArray<Real> &prim, const AthenaArray<Real> &bcc, AthenaArray<Real> &cons)
{
  ...
  for (int k=pmb->ks; k<=pmb->ke; k++) {
	...
	Real a_ph_net = a_r1*sin_zr -sin_ph*a_x + cos_ph*a_y;
	Real a_z_net = a_z_cart + a_r1*sin_zr; //need spherical part? from a_r1?
...
void AGNDiskOuterX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,FaceField &b, Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh){
  int L1flag = 0;
  Real local_dens = 1.0;
  Real local_vr = -0.01;
...
void OutflowInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,FaceField &b, Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh){
  for (int k=ks; k<=ke; ++k) {//z
    for (int j=js; j<=je; ++j) {//phi
      for (int i=1; i<=(NGHOST); ++i) {//R
